...
void FixQEq::init_storage()
{
  nlocal = atom->nlocal;
  nall = atom->nlocal + atom->nghost;
  double *locpot; // Nicolas 
  char tmp1[] = "locpot"; // Nicolas 
  int flag_echemdid; // Nicolas 
  flag_echemdid = get_names(tmp1,locpot); // Nicolas 

  for (int i = 0; i < nall; i++) {
    Hdia_inv[i] = 1. / eta[atom->type[i]];
    if (flag_echemdid == 1) b_s[i] = -chi[atom->type[i]] - locpot[i]; // Nicolas
    else b_s[i] = -chi[atom->type[i]]; // Nicolas
...
int FixQEq::get_names(char *c,double *&ptr)  // Nicolas
{
 int index,flag,cols;
 int flag_echemdid = 1;
